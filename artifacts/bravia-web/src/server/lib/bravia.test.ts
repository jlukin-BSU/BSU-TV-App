...
import { BraviaError, setInput, getPowerStatus, getApplicationList } from "./bravia";
import type { Display } from "./config";

/**
 * Characterisation tests for the Sony BRAVIA REST client.
 *
...
describe("request shape", () => {
  ...
  test("authenticates with the per-display PSK header", async () => {
    stubFetch(200, JSON.stringify({ result: [], id: 20 }));
    await setInput(display({ psk: "abc123" }), 3);
  ...
  test("targets the control override when one is set", async () => {
    stubFetch(200, JSON.stringify({ result: [], id: 20 }));
    await setInput(display({ targetIp: "127.0.0.1" }), 3);
...
describe("error handling", () => {
  test("a Sony error tuple in a 200 body is a failure, not a success", async () => {
    ...
    const err = await setInput(display(), 3).then(
      () => null,
      (e: unknown) => e,
    );

    assert.ok(err instanceof BraviaError, "expected a BraviaError");
  ...
  test("403 is reported as a PSK problem", async () => {
    ...
    assert.ok(err instanceof BraviaError);
  ...
  test("a non-JSON body is an error, not a silent empty result", async () => {
    ...
    await assert.rejects(() => setInput(display(), 3), BraviaError);
  });

  test("an unresolved hostname fails before any request is attempted", async () => {
    stubFetch(200, JSON.stringify({ result: [] }));
    const err = await setInput(display({ targetIp: null, resolvedIps: [] }), 3).then(
      () => null,
      (e: unknown) => e,
    );
...
describe("response parsing", () => {
  test("getPowerStatus maps the reported status", async () => {
    ...
    assert.equal(await getPowerStatus(display()), "active");

    stubFetch(200, JSON.stringify({ result: [{ status: "standby" }], id: 50 }));
    assert.equal(await getPowerStatus(display()), "standby");
  });
  ...
  test("getPowerStatus is 'unknown' rather than throwing on an unexpected value", async () => {
    ...
    assert.equal(await getPowerStatus(display()), "unknown");
  });
  ...
  test("getApplicationList tolerates a non-array result", async () => {
    ...
    assert.deepEqual(await getApplicationList(display()), []);
  });
});
...
describe("dry run", () => {
  test("sends nothing and still returns a plausible payload", async () => {
    stubFetch(200, JSON.stringify({ result: [] }));
    const status = await getPowerStatus(display({ dryRun: true }));
