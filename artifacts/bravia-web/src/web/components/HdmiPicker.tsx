...
import { switchInput } from "../lib/api";
import type { InputEntry } from "../../shared/catalog";

/**
 * HDMI input picker. Ported from artifacts/bsu-tv-hub, with the relay call
 * (sendRelayCommand) replaced by the REST backend (switchInput -> /api/input),
 * and the input list now supplied by the server config for this display.
 */
...
export function HdmiPicker({ open, onClose, inputs }: HdmiPickerProps) {
  ...
  async function handleSelect(entry: InputEntry) {
    ...
    try {
      await switchInput(entry.id);
      setTimeout(() => onClose(), 1200);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.warn("Input switch failed:", msg);
      setErrorMsg(msg);
      setState("error");
      setTimeout(() => {
        setState("idle");
  ...
  }, [open, focusIndex, state, onClose, inputs]);
